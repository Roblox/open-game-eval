...
3. **Cleanest tool error rate of the three.** 0.039 errors/eval vs 0.064 (Opus 4.6) vs 0.074 (Opus 4.7). `multi_edit` errors are significantly lower than Opus 4.7 (0.007 vs 0.028, p=0.04). When GPT-5.5 commits to an action, the call tends to be syntactically correct.
...
5. **GPT-5.5 wins 7 evals where Opus 4.6 fails by ≥40pp, loses 15 where Opus 4.6 wins by ≥40pp.** Versus Opus 4.7 the matchup is more even (10 wins to 7 losses by ≥40pp). Net: comparable to Opus 4.7, behind Opus 4.6.
...
GPT-5.5's `multi_edit` error rate is significantly lower than Opus 4.7's (p=0.04). GPT-5.5 matches Opus 4.6's already-low rate while Opus 4.7 regressed on this dimension. Consistent with GPT-5.5 making fewer but better-formed action calls.
...
### Pattern: GPT-5.5 finishes within 5-min timeout where Opus runs out

**`122_animal_item_with_rarity`** is a very long multi-spec request (6 items, 8 rarities, 24 animals, conveyor belts, billboards, exact JSON-style billboard code). Both Opus runs **timed out at 300s** for all 5 attempts. GPT-5.5 finishes in ~6 tool calls per attempt and passes 4/5 with much lower per-step latency. Opus lost this purely to wall-clock; GPT-5.5's terse exploration wins by completion time alone.
...
15 evals. Dominant pattern: **GPT-5.5 didn't gather enough information**.
...
### When all models pass (29 evals, "easy" tasks)
...
Even on the easiest tasks (where everyone passes), GPT-5.5 gets there with 40% fewer tools than Opus 4.6. Structural behavior, not a failure-mode artifact.

### When all models fail (38 evals, "hard" tasks)
...
On hard tasks where all three fail, GPT-5.5 still gives up with fewer attempts than Opus 4.7. The failure profile is "shallow exploration → confident-but-wrong action", a different shape than Opus 4.7's "shallow exploration → ask for clarification" or Opus 4.6's "deep exploration → over-engineered solution."
...
| Tool error rate | 0.064 | 0.074 | **0.039** |
...
GPT-5.5 is **statistically equivalent to Opus 4.7 in pass rate at meaningfully lower cost**, with the cleanest tool error rate of the three. It inherits Opus 4.7's "less-is-more" exploration profile and takes it further, leaning on `script_search` (fuzzy name match) as a cheap proxy for deeper inspection. This works when the target script is well-named (most of `Evals/`) and breaks when the target requires property inspection (`002_emit_white_smoke`, `090_fps_display_target_damage_ui`) or bulk discovery (`049_surburban_fridge_door_open`, `043_platformer_bouncing_jumper`).

Three-way summary on our 87-eval suite:
- **Opus 4.6**: highest pass rate, most expensive, best for hard discovery.
- **Opus 4.7**: middle ground. Best for clean Roblox idioms, narrow well-specified tasks.
- **GPT-5.5**: tied with Opus 4.7 on pass rate, cheaper than either Opus release, lowest tool error rate, weakest on tasks requiring property-level inspection.
